...
def mezclar_cubo(posiciones, colores, pasos=8):
    ...
    history = [("Gira la cara superior a la derecha", "Arriba a la derecha"),
...
st.title("Rubik 3D didáctico + Guardar en MongoDB")

if "cubo3d_pos" not in st.session_state or "cubo3d_col" not in st.session_state or "movs" not in st.session_state:
...
({len(st.session_state.movs) - st.session_state.step} pasos por resolver)")
